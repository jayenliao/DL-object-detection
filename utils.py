...
# Label map
voc_labels = (
    'aquarium', 'bottle', 'bowl', 'box', 'bucket', 'plastic_bag', 'plate',
...
def create_data_lists_subset(subset:str, dtPATH='./data_tables/', labelPATH='./train_cdc/train_annotations/', imgPATH='./train_cdc/train_images/'):
    ...
    with open(dtPATH + 'table_' + subset + '.txt', 'r') as f:
        lines = f.readlines()

    for line in lines:
        id = line.split(' ')[0].split('.')[0]
        fn_id = os.path.join(labelPATH, id + '.xml')
        
        # Parse annotation's XML file
        try:
            objects = parse_annotation(fn_id)
        except:
            print(fn_id)
        ...
        train_images.append(os.path.join(imgPATH, id + '.jpg'))

    assert len(train_objects) == len(train_images)

    # Save to file
    with open(os.path.join(dtPATH, 'images_' + subset + '.json'), 'w') as j:
        json.dump(train_images, j)
    with open(os.path.join(dtPATH, 'objects_' + subset + '.json'), 'w') as j:
        json.dump(train_objects, j)

    print('\nThere are %d images containing a total of %d objects. Files have been saved to %s.' % (
...
def create_data_lists(subsets=['tr', 'va', 'te'], dtPATH='../data_tables/', labelPATH='../train_cdc/train_annotations/', imgPATH='../train_cdc/train_images/'):
    ...
    # Also save label map
    with open(os.path.join(dtPATH, 'label_map.json'), 'w') as j:
...
def save_checkpoint(epoch, model, optimizer, folder:str):
    """
    ...
    if not os.path.exists(folder):
        os.mkdir(folder)
    fn = folder + 'checkpoint_ssd300.pth.tar'
    torch.save(state, fn)
    print('The model checkpoint has been save as')
    print(fn)
...
def save_txt(out:list, savePATH:str, fn:str, verbose=False):
    ...
    with open(FN, 'w') as f:
        for row in out:
            f.write(row + '\n')
    if verbose:
        print('The object is save as', FN)
